...
export default async function ConquistasPage() {
  ...
  return (
    <Pagina>
      <Cabecalho
        etiqueta={`${desbloqueadas} de ${g.conquistas.length} medalhas`}
        titulo="Conquistas"
        descricao="Ganhe XP resolvendo questões e simulados, e mantenha sua sequência de estudos."
      />
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="sm:col-span-2">
          <CardContent className="flex items-center gap-4 p-5">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-sm bg-indigo-500/15 text-xl font-bold text-indigo-300">
              {g.nivel}
            </div>
        ...
        <Card>
          <CardContent className="flex items-center gap-4 p-5">
            <div
              className={cn(
                "flex h-14 w-14 shrink-0 items-center justify-center rounded-sm",
          ...
          {g.conquistas.map((c) => {
            ...
            return (
              <div
                key={c.id}
                className={cn(
                  "flex min-w-0 gap-3 rounded-sm border p-4 transition-colors",
                  c.desbloqueada
