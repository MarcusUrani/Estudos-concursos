...
import { listarLegislacoes } from "@/server/legislacao";
...
export default async function EstudarPage() {
  const legislacoes = await listarLegislacoes();
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <header>
        ...
        <p className="text-sm text-slate-400">
          Cada tema reúne um resumo (derivado das questões), a leitura dos pontos-chave e a prática
          das questões relacionadas. Escolha por onde começar.
        </p>
      </header>
              ...
              <Card className="h-full transition-colors group-hover:border-indigo-600/50">
                <CardContent className="flex h-full flex-col gap-3 p-5">
                  ...
                  <div className="flex-1">
                    <h2 className="font-semibold leading-tight text-slate-100">{l.nome}</h2>
                    {l.descricao && (
